...
Normally this is just because of a temporary glitch in the data stream: often one of the river sensors will go offline briefly. This usually resolves quickly but can take a while: it will just display the most recently created forecast in the meantime. Also, the code to update sometimes can sit in a queue for a while on the GitHub servers, which is why it doesn't update on the hour every hour.
