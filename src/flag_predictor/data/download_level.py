"""
Download historical qualified level (river height) data from Environment Agency Hydrology API.
...
# Mapping of level station names to Hydrology API measure IDs (full notation)
# Format: {station-uuid}-level-i-900-m-qualified for 15-minute instantaneous level
LEVEL_HYDROLOGY_MEASURES = {
    'Hannington-Bridge': '62a5f097-03e7-4dca-91cc-cb3cf6f1c1c7-level-i-900-m-qualified',
    'Shifford-Lock': 'b94cc43a-f693-4efa-8749-c4718c7eb2b8-level-i-900-m-qualified',
    'Northmoor-Lock': '604b2518-433e-464c-a0c4-f34dcf85d3ac-level-i-900-m-qualified',
    'Eynsham-Lock': 'dd230984-b45b-4fd6-8ebf-45ca375799f1-level-i-900-m-qualified',
}

BASE_URL = "https://environment.data.gov.uk/hydrology/id/measures"


def download_station_level(
    station_name: str,
    measure_id: str,
    ...
    """
    Download qualified level data for a single station as CSV.
    
    Args:
        station_name: Name of the level station
        ...
        output_dir: Directory to save CSV file (defaults to data/level_training_data/)
    ...
    if output_dir is None:
        output_dir = project_root / 'data' / 'level_training_data'
    else:
        output_dir = Path(output_dir)
    
    output_dir.mkdir(parents=True, exist_ok=True)
    
    # Build CSV API URL for level data
    # Level endpoint pattern: {measure_id}/readings.csv
    # measure_id should be full notation like: {uuid}-level-i-900-m-qualified
    base_url = f"{BASE_URL}/{measure_id}/readings.csv"
    
    if verbose:
        print(f"Downloading {station_name} level...")
        if min_date:
    ...
    try:
        output_file = output_dir / f"{station_name}-level-15min-Qualified.csv"
...
def download_all_level(
    ...
    """
    Download qualified level data for all stations.
    
    Args:
        stations: Dictionary mapping station names to measure IDs
                 (defaults to LEVEL_HYDROLOGY_MEASURES)
        min_date: Minimum date in YYYY-MM-DD format (filters at API level)
        max_date: Maximum date in YYYY-MM-DD format (optional)
    ...
    """
    if stations is None:
        stations = LEVEL_HYDROLOGY_MEASURES
    ...
    if verbose:
        print(f"\n{'='*70}")
        print(f"Downloading historical qualified level data")
        print(f"{'='*70}")
        if min_date:
    ...
    for name, measure_id in stations.items():
        ...
        success = download_station_level(
            station_name=name,
...
if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser(description='Download historical qualified level data from Environment Agency Hydrology API')
    ...
    parser.add_argument('--output-dir', type=str, help='Output directory for CSV files (defaults to data/level_training_data/)')
    ...
    if args.station:
        if args.station not in LEVEL_HYDROLOGY_MEASURES:
            print(f"Error: Unknown station '{args.station}'. Available: {', '.join(LEVEL_HYDROLOGY_MEASURES.keys())}")
            exit(1)
        stations_to_download = {args.station: LEVEL_HYDROLOGY_MEASURES[args.station]}
    else:
        stations_to_download = LEVEL_HYDROLOGY_MEASURES
