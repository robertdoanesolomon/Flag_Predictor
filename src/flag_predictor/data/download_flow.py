"""
Download historical qualified flow data from Environment Agency Hydrology API.

Uses the CSV endpoint with date filtering:
https://environment.data.gov.uk/hydrology/id/measures/{measure-id}-flow-i-15_min-m3s-qualified/readings.csv?min-date=YYYY-MM-DD
"""
...
# Mapping of flow station names to Hydrology API measure IDs (full notation)
# Format: {station-uuid}-flow-i-900-m3s-qualified for 15-minute instantaneous flow
FLOW_HYDROLOGY_MEASURES = {
    'Farmoor': '6b80bd1c-6c19-4c26-875d-108bf8ef1a84-flow-i-900-m3s-qualified',
}

BASE_URL = "https://environment.data.gov.uk/hydrology/id/measures"


def download_station_flow(
    station_name: str,
    measure_id: str,
    ...
    """
    Download qualified flow data for a single station as CSV.
    
    Args:
        station_name: Name of the flow station
        ...
        output_dir: Directory to save CSV file (defaults to data/flow_training_data/)
    ...
    if output_dir is None:
        output_dir = project_root / 'data' / 'flow_training_data'
    else:
        output_dir = Path(output_dir)
    
    output_dir.mkdir(parents=True, exist_ok=True)
    
    # Build CSV API URL for flow data
    # Flow endpoint pattern: {measure_id}/readings.csv
    # measure_id should be full notation like: {uuid}-flow-i-900-m3s-qualified
    base_url = f"{BASE_URL}/{measure_id}/readings.csv"
    
    if verbose:
        print(f"Downloading {station_name} flow...")
        if min_date:
    ...
    try:
        output_file = output_dir / f"{station_name}-flow-15min-Qualified.csv"
...
def download_all_flow(
    ...
    """
    Download qualified flow data for all stations.
    
    Args:
        stations: Dictionary mapping station names to measure IDs
                 (defaults to FLOW_HYDROLOGY_MEASURES)
        min_date: Minimum date in YYYY-MM-DD format (filters at API level)
        max_date: Maximum date in YYYY-MM-DD format (optional)
    ...
    """
    if stations is None:
        stations = FLOW_HYDROLOGY_MEASURES
    ...
    if verbose:
        print(f"\n{'='*70}")
        print(f"Downloading historical qualified flow data")
        print(f"{'='*70}")
        if min_date:
...
if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser(description='Download historical qualified flow data from Environment Agency Hydrology API')
    ...
    parser.add_argument('--output-dir', type=str, help='Output directory for CSV files (defaults to data/flow_training_data/)')
    ...
    if args.station:
        if args.station not in FLOW_HYDROLOGY_MEASURES:
            print(f"Error: Unknown station '{args.station}'. Available: {', '.join(FLOW_HYDROLOGY_MEASURES.keys())}")
            exit(1)
        stations_to_download = {args.station: FLOW_HYDROLOGY_MEASURES[args.station]}
    else:
        stations_to_download = FLOW_HYDROLOGY_MEASURES
